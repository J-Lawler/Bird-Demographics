...
parameters{
  ...
  vector[n_months] b_mon_p; // month parameter
  vector[n_winters] b_wint_p; // winter year parameter
  ...
  real mu_mon_p;
  real<lower=0> sig_mon_p;
  
  real mu_wint_p;
  real<lower=0> sig_wint_p;
...
model{
  ...
  b_mon_p ~ normal(mu_mon_p,sig_mon_p);
  b_wint_p ~ normal(mu_wint_p,sig_wint_p);
  ...
  sig_mon_phi ~ normal(1,0.25);
  
  mu_wint_phi ~ normal(0,0.5);
  sig_wint_phi ~ normal(1,0.25);
  ...
  mu_mon_p ~ normal(0,0.5);
  sig_mon_p ~ normal(1,0.25);
  
  mu_wint_p ~ normal(0,0.5);
  sig_wint_p ~ normal(1,0.25);
  ...
  vector[n_chi] lin_p_chi = a_spec_p[species_chi] + b_adult_p*vec_p_adult_chi 
                      + b_mon_p[month_chi] + b_wint_p[winter_chi];
...
if(prior == 1){
  ...
  phi_prior = inv_logit(a_spec_phi[species_prior] 
                                ...
                                + b_mon_phi[month_prior] 
                                + b_wint_phi[winter_prior]);
                              
  // Psi

  psi_prior = inv_logit(a_spec_psi[species_prior] 
                                + b_wint_psi[winter_prior]);
...
prior_list <- outputs_D$stan_list

# Permit generation of prior predictive
prior_list$prior <- 1


if(!file.exists("Models/samples_m_D_prior.rds")){
  
  # Compiles the model
  stan_m_D <- stan_model(model_name = "stan_m_D",model_code=code_m_D)
  
  # Sample
  samples_m_D_prior <-  sampling(stan_m_D,
                                 data = prior_list, 
                                 chains=1, iter = 10000,
  ...
  # Save file
  saveRDS(samples_m_D_prior,file="Models/samples_m_D_prior.rds")
}else{
...
# Phi
gather_draws(samples_m_D_prior, 
             phi_prior[i])|>
  ggplot(aes(x = .value))+
...
posterior_list <- outputs_D$stan_list
...
if(!file.exists("Models/samples_m_D_posterior.rds")){
  ...
  start_time <- Sys.time()
  # Sample
  samples_m_D_posterior <-  sampling(stan_m_D,
                                     data = posterior_list, 
                                     chains=4, iter = 2000,
                                     cores = parallel::detectCores(),
                                     #thin = 10,
  ...
  # Save file
  saveRDS(samples_m_D_posterior,file="Models/samples_m_D_posterior.rds")
}else{
...
# Phi
gather_draws(samples_m_D_posterior, 
             a_phi)|>
