...
model{
  ...
  vector[n_chi] lin_phi_chi = a_spec_phi[species_chi];
  ...
  vector[n_chi] lin_p_chi = a_spec_p[species_chi];
...
outputs_B <- f_prep_for_stan(df_model)
...
prior_list <- outputs_B$stan_list

# Permit generation of prior predictive
prior_list$prior <- 1


if(!file.exists("Models/samples_m_B_prior.rds")){
  
  # Compiles the model
  stan_m_B <- stan_model(model_name = "stan_m_B",model_code=code_m_B)
  
  # Sample
  samples_m_B_prior <-  sampling(stan_m_B,
                                 data = prior_list, 
                                 chains=1, iter = 10000,
  ...
  # Save file
  saveRDS(samples_m_B_prior,file="Models/samples_m_B_prior.rds")
}else{
  samples_m_B_prior <- readRDS(file="Models/samples_m_B_prior.rds")
}
...
# Phi
gather_draws(samples_m_B_prior, 
             phi_prior[i])|>
  ggplot(aes(x = .value))+
...
posterior_list <- outputs_B$stan_list
...
if(!file.exists("Models/samples_m_B_posterior.rds")){
  ...
  start_time <- Sys.time()
  # Sample
  samples_m_B_posterior <-  sampling(stan_m_B,
                                     data = posterior_list, 
                                     chains=4, iter = 2000,
                                     cores = parallel::detectCores(),
                                     #thin = 10,
  ...
  # Save file
  saveRDS(samples_m_B_posterior,file="Models/samples_m_B_posterior.rds")
}else{
...
# Phi
gather_draws(samples_m_B_posterior, 
             a_phi)|>
